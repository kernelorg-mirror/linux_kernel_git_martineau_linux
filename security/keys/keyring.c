				  ...
				  struct key_preparsed_payload *prep)
{
	...
	struct key_restriction *(*lookup_restrict)(char *params) = NULL;
	int ret = -EINVAL;
	const char restrict_prefix[] = "restrict=";
	...
	datablob += strlen(restrict_prefix);
	...
	lookup_restrict = restrict_type->lookup_restrict;

	if (!lookup_restrict)
		goto error;

	restrict_link = lookup_restrict(datablob);
