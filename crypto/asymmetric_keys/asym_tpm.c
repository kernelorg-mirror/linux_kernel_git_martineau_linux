...
{
	pr_info("encoding: %s, hash_algo: %s\n",
		encoding ? encoding : "", hash_algo ? hash_algo : hash_algo);
...
static int extract_key_parameters(struct tpm_key *tk)
{
	...
	pr_info("Key length: %d\n", key_len);
	pr_info("Public Key Length: %d\n", sz);
	tk->key_len = key_len;
	tk->pub_key = pub_key;
	tk->pub_key_len = sz;
